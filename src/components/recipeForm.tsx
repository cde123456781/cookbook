...
export default function NewRecipeForm({
  ...
  const [title, setTitle] = useState("");
    ...
    return (
    <div className="flex h-[80vh] items-center justify-center">
        <p className="text-gray-500">Please <Link href="/login">login</Link> to add a new recipe to your cookbook</p>
      </div>
    );
  }
  ...
  <fieldset className="fieldset flex flex-col bg-base-200 border-base-300 rounded-box w-full max-w-4xl border p-10">
    <label className="fieldset-legend text-lg">Add Recipe</label>

    {/* Recipe Details */}
    <label className="label text-base mt-4">Recipe Title</label>
    <input
      type="text"
    ...
    <label className="label text-base mt-4">Duration</label>
    <input
      type="text"
      className="input w-full"
    ...
    <label className="label text-base mt-4">Description</label>
    <textarea
    ...
    <label className="label text-base mt-4">Recipe Image URL</label>
    <input
      type="text"
      className="input w-full"
      placeholder="https://..."
...
{recipeIngredients.map((ingredient, index) => (
  ...
  >
    ...
    <Select options={ingredients} value={null} onChange={function (newValue: SingleValue<{ id: number; name: string; }>, actionMeta: ActionMeta<{ id: number; name: string; }>): void {
            throw new Error("Function not implemented.");
        } } inputValue={""} onInputChange={function (newValue: string, actionMeta: InputActionMeta): void {
            throw new Error("Function not implemented.");
...
>
  Add Ingredient
    ...
    {steps.map((step, index) => (
      <div key={index} className="card bg-base-100 border p-4 mb-4">
